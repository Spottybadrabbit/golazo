    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-line bg-night/80 backdrop-blur-md">
      <nav className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/play"
            className="hidden rounded-full bg-scarlet px-4 py-2 text-sm font-bold text-chalk transition-transform hover:scale-[1.03] active:translate-y-px md:block"
          >
            Start a streak
          </Link>
        </div>
...
export function BallMark({ size = 26 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 26 26" fill="none" aria-hidden="true">
      <circle cx="13" cy="13" r="11.5" stroke="#F5F2E8" strokeWidth="2" />
      <path
